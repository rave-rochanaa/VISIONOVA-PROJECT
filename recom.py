...
# --- Professional UI with Enhanced Typography and Spacing ---
PROFESSIONAL_CSS = """
...
/* Form elements */
.stSelectbox, .stTextInput, .stSlider, .stFileUploader {
    margin-bottom: 1.25rem;
}
...
st.markdown("Transform your data into actionable insights with AI-powered dashboard recommendations")
...
with st.sidebar:
    ...
    # Preference sliders
    trends_pref = st.slider("*Trend Analysis Preference*", 1, 10, st.session_state.user_preferences['trends'],
                           help="How important are trend visualizations to you?")
    comparisons_pref = st.slider("*Comparison Preference*", 1, 10, st.session_state.user_preferences['comparisons'],
                                help="How important are comparison charts to you?")
...
with tab1:
    ...
    st.markdown("Start by uploading your CSV file to get personalized dashboard recommendations")
    ...
    if uploaded_file is not None:
        try:
            ...
            with col3:
                st.metric("*Numeric Columns*", dataset_features['num_numeric'])
            with col4:
                st.metric("*Categorical Columns*", dataset_features['num_categorical'])
            ...
            with feature_cols[0]:
                ...
                if dataset_features['has_time']:
                    st.success("⏰ *Time-based data detected*")
...
with tab2:
    ...
    if st.session_state.current_dataset is not None:
        with st.spinner("Analyzing your dataset for recommendations..."):
        ...
        if recommendations:
            ...
            st.markdown(f"""
            <div class="card" style="border-left: 4px solid {layout_info['color']};">
                <div style="display: flex; align-items: center; margin-bottom: 1rem;">
                    <div style="font-size: 2rem; margin-right: 1rem;">{layout_info['icon']}</div>
                    <div>
                        <h3 style="margin: 0;">{layout_name}</h3>
                        <p style="margin: 0; color: var(--text-light);">{layout_info['description']}</p>
                    </div>
                </div>
                <div style="display: flex; justify-content: space-between; align-items: center;">
                    <div>
                        <p style="font-size: 0.9rem; margin-bottom: 0.5rem;"><strong>Confidence:</strong> {confidence:.0%}</p>
                        <p style="font-size: 0.9rem; margin: 0;"><strong>Recommended Charts:</strong></p>
                        <ul style="margin-top: 0.25rem; margin-bottom: 0; padding-left: 1.25rem;">
                            {''.join([f'<li>{chart}</li>' for chart in layout_info['charts'][:3]])}
                        </ul>
                    </div>
                    <div>
                        <button style="background: {layout_info['color']}; border: none; color: white; padding: 0.5rem 1rem; border-radius: 6px; cursor: pointer; font-weight: 600;"
                            onclick="document.getElementById('apply_{layout_name}').click()">Apply Dashboard</button>
                    </div>
                </div>
            </div>
            """, unsafe_allow_html=True)
            
            if st.button(f"Apply {layout_name}", key=f"apply_{layout_name}", use_container_width=True):
                    ...
                    with st.container():
                        ...
                        <div class="card">
                            <div style="text-align: center; margin-bottom: 1rem;">
                                <div style="font-size: 2rem;">{layout_info['icon']}</div>
                                <h4 style="margin: 0.5rem 0;">{layout_name}</h4>
                                <div style="height: 6px; background: #e2e8f0; border-radius: 3px; margin: 0.5rem 0;">
                                    <div style="height: 100%; width: {score*100}%; background: {layout_info['color']}; border-radius: 3px;"></div>
                                </div>
                                <small>Confidence: {score:.0%}</small>
                            </div>
                            <p style="font-size: 0.9rem; margin-bottom: 1rem;">{layout_info['description']}</p>
                        </div>
                        """, unsafe_allow_html=True)
                        
                        if st.button("Apply", key=f"apply_{layout_name}_{i}", use_container_width=True):
                            st.session_state.current_layout = layout_name
                            logger.log_event('layout_applied', {'layout_name': layout_name}, dataset_features)
                            st.success(f"✅ Applied {layout_name} layout!")
                            st.session_state.viewing_start = datetime.now()
            ...
            if dataset_features.get('key_metrics'):
                ...
                metric_cols = st.columns(len(dataset_features['key_metrics']), gap="medium")
                ...
                st.warning("No key metrics identified. Please analyze individual metrics in the Analyze Metrics tab.")
                        ...
                        with st.container():
                            ...
                            <div class="card">
                                <div style="display: flex; align-items: center; margin-bottom: 0.75rem;">
                                    <div style="font-size: 1.5rem; margin-right: 0.75rem;">{rec['icon']}</div>
                                    <h4 style="margin: 0;">{rec['type']}</h4>
                                </div>
                                <p style="font-size: 0.9rem; margin: 0;">{rec['description']}</p>
                            </div>
                            """, unsafe_allow_html=True)
            else:
    ...
    if st.session_state.current_dataset is not None:
        ...
        with col2:
            ...
            if dataset_features.get('dimensions'):
                ...
                if selected_dimension and selected_dimension != 'None':
        ...
        if selected_measure:
            ...
            if 'analysis_results' not in st.session_state or st.session_state.get('last_analysis') != (selected_measure, selected_dimension, tuple(dimension_values)):
                with st.spinner("Analyzing metric..."):
            ...
            if st.session_state.analysis_results:
                ...
                for i, (label, value) in enumerate(stats):
                    with stats_cols[i % 4]:
                        st.metric(f"*{label}*", f"{value:,.2f}" if isinstance(value, float) else value)
                ...
                if 'trend_analysis' in results:
                    ...
                    with trend_cols[0]:
                        st.metric("*Overall Trend*", results['trend_analysis']['trend'].capitalize())
                    with trend_cols[1]:
                ...
                if 'dimension_analysis' in results:
                    st.divider()
                    st.markdown(f"#### Dimension Analysis: '{selected_dimension}'")
                    ...
                    st.dataframe(dim_df.style.format({col: "{:,.2f}" for col in dim_df.select_dtypes(include='number').columns}),use_container_width=True)
                    # st.dataframe(dim_df.style.format("{:,.2f}"), use_container_width=True)
                    formatted_df = dim_df.style.format({col: "{:,.2f}" for col in dim_df.select_dtypes(include='number').columns})
        ...
        if "ROI" in st.session_state.current_layout:
            ...
            with col1:
                ...
                if len(df.select_dtypes(include=[np.number]).columns) >= 2:
                    ...
                    if st.button("Analyze Trend", key="analyze_trend", use_container_width=True):
            ...
            with col2:
                ...
                if len(df.select_dtypes(include=[np.number]).columns) >= 1:
                    ...
                    st.plotly_chart(fig, use_container_width=True)
                    if st.button("View Distribution", key="view_dist", use_container_width=True):
            ...
            if len(df.select_dtypes(include=[np.number]).columns) >= 1:
                ...
                for col in numeric_cols[:2]:
                    ...
                    st.plotly_chart(fig, use_container_width=True)
                    if st.button(f"Analyze {col}", key=f"analyze_{col}", use_container_width=True):
            ...
            with col1:
                ...
                if len(cat_cols) > 0:
                    ...
                    st.plotly_chart(fig, use_container_width=True)
                    if st.button(f"Explore {cat_col}", key=f"explore_{cat_col}", use_container_width=True):
            ...
            with col2:
                ...
                if len(num_cols) > 0:
                    ...
                    st.plotly_chart(fig, use_container_width=True)
                    if st.button(f"Analyze {num_col}", key=f"box_{num_col}", use_container_width=True):
            ...
            with col1:
                ...
                if len(numeric_cols) >= 1:
                    ...
                    st.plotly_chart(fig, use_container_width=True)
                    if st.button(f"Analyze {numeric_col}", key=f"bar_{numeric_col}", use_container_width=True):
            ...
            with col2:
                ...
                if len(cat_cols) >= 1:
                    ...
                    st.plotly_chart(fig, use_container_width=True)
                    if st.button(f"Explore {cat_col}", key=f"count_{cat_col}", use_container_width=True):
...
with tab5:
    ...
    if st.session_state.user_logs:
        ...
        for log in st.session_state.user_logs[-5:]:
            ts = datetime.fromisoformat(log.get('timestamp')).strftime("%Y-%m-%d %H:%M") if log.get('timestamp') else 'N/A'
            etype = log.get('event_type', 'N/A')
            details = log.get('details', {})
            
            with st.expander(f"{ts} - {etype.replace('_', ' ').title()}"):
